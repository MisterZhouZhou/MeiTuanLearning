...
import {StyleSheet, Platform, View, Text, Image, TouchableHighlight} from 'react-native';
...
export default class JZDiscount extends Component{
  constructor(props){
    super(props);
    this.state={
      isLoading:false,
  ...
  render(){
     if (this.state.dataSource === null) {
      return (<Text>吃肉都不开心的话，还不如吃酸菜。</Text>);
    }else {
      return (
    ...
    var group = data.map(function(item,i){
      ...
      return (
        <TouchableHighlight key = {i}
          style = {styles.touch}
          onPress = {() => weakself.props.onSelect1(weburl)}>
